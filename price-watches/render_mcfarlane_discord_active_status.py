...
from mcfarlane_wallet_aliases import last_sale_text, listing_price_text, seller_label
...
for c in active:
    ...
    prices = ", ".join(
        f"{listing_price_text(x, state)} by {seller_label(x)} (set {x.get('price_set_time') or 'time not recorded'})"
        for x in listings
    ) or "—"
